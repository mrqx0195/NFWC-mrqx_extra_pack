...
EntityEvents.spawned(event => {
    ...
    */
    ...
    if (!player) return;
    if(player.stages.has('champion_control_disable')) return;
    let championLevel = player.stages.getAll().toArray().find(ele => ele.startsWith('champion_control_'))
    if(!championLevel){ championLevel = 'champion_control_default'}
    let matchResult = championLevel.match('champion_control_(\\d+)');
    let championOdds = matchResult ? matchResult[1] : null;
    if(!championOdds && !(championLevel == 'champion_control_default' || championLevel == 'champion_control_warponly'))
        {championLevel = 'champion_control_default'}
    if(championLevel == 'champion_control_default'){
        if(player.hasEffect('luna_flesh_reforged:warpward')) return;}
    if(championLevel == 'champion_control_default' || championLevel == 'champion_control_warponly'){
        let warp = player.persistentData.getInt(warpCount)
        if (warp < 20) return
        if (Math.random() > 0.001 * warp) return}
    if(championOdds){
        if(Math.random()*100 > championOdds) return;
    }
    if(entity.persistentData.get('champion'))return;
